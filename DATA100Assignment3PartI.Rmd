...
convert_latlon <- function(latlon) {
    # YOUR CODE HERE
  val <- parse_number(latlon)
  sign <- if_else(str_detect(latlon, "[WS]$"), -1, 1)
  sign * val
}
...
aware_pop <- aware_fixed_country_match |>
  inner_join(pop_2020, by = "country") |>
  select(
...
covid_2020_match_country <- covid_2020 |>
  mutate(
    country = case_when(
      ...
      country == "Micronesia" | country == "Micronesia (country)" ~ "Micronesia, Fed. Sts.",
